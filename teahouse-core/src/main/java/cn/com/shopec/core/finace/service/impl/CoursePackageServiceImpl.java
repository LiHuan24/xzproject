...
import cn.com.shopec.core.finace.dao.CoursePackageDao;
...
public class CoursePackageServiceImpl implements CoursePackageService {
	...
	private static final Log log = LogFactory.getLog(CoursePackageServiceImpl.class);
	...
	public List<CoursePackage> getCoursePackageList(Query q) {
		List<CoursePackage> list = null;
		try {
			// 已删除的不查出
			CoursePackage coursePackage = (CoursePackage) q.getQ();
			if (coursePackage != null) {
				coursePackage.setIsDeleted(Constant.DEL_STATE_NO);
			}

			// 直接调用Dao方法进行查询
			list = coursePackageDao.queryAll(q);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
		}
		// 如list为null时，则改为返回一个空列表
		list = list == null ? new ArrayList<CoursePackage>(0) : list;
		return list;
	}
	...
	public PageFinder<CoursePackage> getCoursePackagePagedList(Query q) {
		PageFinder<CoursePackage> page = new PageFinder<CoursePackage>();
		...
		try {
			...
			// 调用dao查询满足条件的分页数据
			list = coursePackageDao.pageList(q);
			// 调用dao统计满足条件的记录总数
			rowCount = coursePackageDao.count(q);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
		}
	...
	public List<CoursePackage> getCoursePackageByIds(String[] ids) {
		...
		if (ids == null || ids.length == 0) {
			...
			try {
				// 调用dao，根据主键数组查询
				list = coursePackageDao.getByIds(ids);
			} catch (Exception e) {
				log.error(e.getMessage(), e);
			}
		}
	...
	public ResultInfo<CoursePackage> delCoursePackage(String id, Operator operator) {
		ResultInfo<CoursePackage> resultInfo = new ResultInfo<CoursePackage>();
		...
		try {
			...
			coursePackage.setCoursePackageNo(id);
			coursePackage.setIsDeleted(Constant.DEL_STATE_YES);
			coursePackage.setUpdateTime(new Date());
			if (operator != null) { // 最近操作人
				coursePackage.setOperatorType(operator.getOperatorType());
				coursePackage.setOperatorId(operator.getOperatorId());
			}

			// 调用Dao执行更新操作，并判断更新语句执行结果
			int count = coursePackageDao.update(coursePackage);
			if (count > 0) {
				resultInfo.setCode(Constant.SECCUESS);
			} else {
				resultInfo.setCode(Constant.FAIL);
			}
			resultInfo.setData(coursePackage);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
	...
	public ResultInfo<CoursePackage> addCoursePackage(CoursePackage coursePackage, Operator operator) {
		...
		if (coursePackage == null) { // 传入参数无效时直接返回失败结果
			resultInfo.setCode(Constant.FAIL);
			resultInfo.setMsg(Constant.ERR_MSG_INVALID_ARG);
			log.info(Constant.ERR_MSG_INVALID_ARG + " coursePackage = " + coursePackage);
		} else {
			try {
				// 如果传入参数的主键为null，则调用生成主键的方法获取新的主键
				if (coursePackage.getCoursePackageNo() == null) {
					coursePackage.setCoursePackageNo(this.generatePK());
				}
				...
				// 设置创建时间和更新时间为当前时间
				Date now = new Date();
				coursePackage.setCreateTime(now);
				coursePackage.setUpdateTime(now);

				// 填充默认值
				this.fillDefaultValues(coursePackage);

				// 调用Dao执行插入操作
				coursePackageDao.add(coursePackage);
				resultInfo.setCode(Constant.SECCUESS);
				resultInfo.setData(coursePackage);
			} catch (Exception e) {
				log.error(e.getMessage(), e);
	...
	public ResultInfo<CoursePackage> updateCoursePackage(CoursePackage coursePackage, Operator operator) {
		...
		if (coursePackage == null || coursePackage.getCoursePackageNo() == null) { // 传入参数无效时直接返回失败结果
			resultInfo.setCode(Constant.FAIL);
			resultInfo.setMsg(Constant.ERR_MSG_INVALID_ARG);
			log.info(Constant.ERR_MSG_INVALID_ARG + " coursePackage = " + coursePackage);
		} else {
	...
	public String generatePK() {
		return "K" + primarykeyService.getValueByBizType(PrimarykeyConstant.coursePackage);
	}
