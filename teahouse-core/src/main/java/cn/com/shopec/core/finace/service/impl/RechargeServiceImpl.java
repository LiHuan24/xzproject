...
import cn.com.shopec.core.finace.dao.RechargeDao;
...
public class RechargeServiceImpl implements RechargeService {
	...
	public List<Recharge> getRechargeList(Query q) {
		...
		try {
			// 已删除的不查出
			Recharge recharge = (Recharge) q.getQ();
			if (recharge != null) {
				recharge.setIsDeleted(Constant.DEL_STATE_NO);
			}

			// 直接调用Dao方法进行查询
			list = rechargeDao.queryAll(q);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
		}
		// 如list为null时，则改为返回一个空列表
		list = list == null ? new ArrayList<Recharge>(0) : list;
		return list;
	}
	...
	public PageFinder<Recharge> getRechargePagedList(Query q) {
		PageFinder<Recharge> page = new PageFinder<Recharge>();
		...
		try {
			...
			// 调用dao查询满足条件的分页数据
			list = rechargeDao.pageList(q);
			// 调用dao统计满足条件的记录总数
			rowCount = rechargeDao.count(q);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
		}
	...
	public ResultInfo<Recharge> delRecharge(String id, Operator operator) {
		ResultInfo<Recharge> resultInfo = new ResultInfo<Recharge>();
		...
		try {
			...
			recharge.setRechargeNo(id);
			recharge.setIsDeleted(Constant.DEL_STATE_YES);
			recharge.setUpdateTime(new Date());
			if (operator != null) { // 最近操作人
				recharge.setOperatorType(operator.getOperatorType());
				recharge.setOperatorId(operator.getOperatorId());
			}

			// 调用Dao执行更新操作，并判断更新语句执行结果
			int count = rechargeDao.update(recharge);
			if (count > 0) {
				resultInfo.setCode(Constant.SECCUESS);
			} else {
				resultInfo.setCode(Constant.FAIL);
			}
			resultInfo.setData(recharge);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
	...
	public ResultInfo<Recharge> addRecharge(Recharge recharge, Operator operator) {
		...
		if (recharge == null) { // 传入参数无效时直接返回失败结果
			resultInfo.setCode(Constant.FAIL);
			resultInfo.setMsg(Constant.ERR_MSG_INVALID_ARG);
			log.info(Constant.ERR_MSG_INVALID_ARG + " recharge = " + recharge);
		} else {
			try {
				// 如果传入参数的主键为null，则调用生成主键的方法获取新的主键
				if (recharge.getRechargeNo() == null) {
					recharge.setRechargeNo(this.generatePK());
				}
				...
				// 填充默认值
				this.fillDefaultValues(recharge);
	...
	public ResultInfo<Recharge> updateRecharge(Recharge recharge, Operator operator) {
		...
		if (recharge == null || recharge.getRechargeNo() == null) { // 传入参数无效时直接返回失败结果
			resultInfo.setCode(Constant.FAIL);
			resultInfo.setMsg(Constant.ERR_MSG_INVALID_ARG);
			log.info(Constant.ERR_MSG_INVALID_ARG + " recharge = " + recharge);
		} else {
	...
	public String generatePK() {
		return "C" + primarykeyService.getValueByBizType(PrimarykeyConstant.recharge);
	}
