...
public class CouponServiceImpl implements CouponService {
		...
		if (coupon == null || coupon.getPlanNo() == null || coupon.getPlanNo() == "") { // 传入参数无效时直接返回失败结果
			...
			if(couponPlan  != null){
				try {
					...
					couponPlan.setAvailableTime1(couponPlan.getAvailableTime1());
					couponPlan.setAvailableTime2(couponPlan.getAvailableTime2());
	...
	public ResultInfo<Coupon> addCoupon(Coupon coupon, String[] memberArray, Operator operator) {
		ResultInfo<Coupon> resultInfo = new ResultInfo<Coupon>();
		if (coupon == null || coupon.getPlanNo() == null || "".equals(coupon.getPlanNo()) || memberArray == null || memberArray.length == 0) { // 传入参数无效时直接返回失败结果
			resultInfo.setCode(Constant.FAIL);
			resultInfo.setMsg(Constant.ERR_MSG_INVALID_ARG);
			log.info(Constant.ERR_MSG_INVALID_ARG + " coupon = " + coupon);
		} else {
			...
			if(couponPlan  != null){
				try {
					...
					existingQuantity = existingQuantity + memberArray.length;
					...
					if (couponPlan.getAvailableTime1() != null) {
					...
					for (int i = 0; i <memberArray.length; i++) {
						...
						if(couponPlan.getAvailableDays()  != null ){
							...
							couponTemp.setAvailableTime2(ECDateUtils.getDateAfter(date, couponPlan.getAvailableDays()));
						}else{
						...
						//保存健身时长  和  课程体验券节数
						couponTemp.setFreeFitnessTime(couponPlan.getFreeFitnessTime());
						couponTemp.setFreeCourseNumber(couponPlan.getFreeCourseNumber());
