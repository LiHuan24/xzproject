...
public class DepositRefundServiceImpl implements DepositRefundService {
	...
	public List<DepositRefund> getDepositRefundListForManagePage(Query query) {
		...
		try {
			// 直接调用Dao方法进行查询
			list = depositRefundDao.getDepositRefundListForManagePage(query);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
		}
		// 如list为null时，则改为返回一个空列表
	 ...
	 */
	@Override
	public ResultInfo<DepositRefund> refundDepositByOffline(DepositRefund depositRefund) {
		ResultInfo<DepositRefund> resultInfo = new ResultInfo<DepositRefund>();

		if (depositRefund != null) {
			depositRefund.setRefundFlowNo("XX" + depositRefund.getRefundNo());// 退款流水号
			depositRefund.setRefundTime(new Date());
			depositRefund.setRefundStatus(1);// 退款状态1.已退款
			depositRefund.setRefundMethod(4);
			depositRefund.setUpdateTime(new Date());
			depositRefundDao.update(depositRefund);

			DepositOrder dOrder = depositOrderDao.get(depositRefund.getDepositOrderNo());
			if (dOrder != null) {
				dOrder.setRemainAmount(
						ECNumberUtils.roundDoubleWithScale(dOrder.getRemainAmount() - dOrder.getFrozenAmount(), 2));
				if (dOrder.getRefundAmount() == null) {
					dOrder.setRefundAmount(0d);
				}
				dOrder.setRefundAmount(
						ECNumberUtils.roundDoubleWithScale(dOrder.getRefundAmount() + dOrder.getFrozenAmount(), 2));
				;
				...
				if (member != null) {
					...
					accounts.setBusinessNo(depositRefund.getRefundNo());
					accounts.setBusinessType(4);
					accounts.setAccountType(2);
					accounts.setAccountMoney(depositRefund.getRefundAmount());
	...
	public ResultInfo<DepositRefund> refundDepositByWX(DepositRefund dRefund, DepositOrder dOrder) {
		...
		Double refundFee = dOrder.getDepositAmount() * 100;
		...
		try {
			Map<String, Object> map = wxPayRefundRequest(transaction_id, out_trade_no, out_refund_no, total_fee,
					refund_fee, op_user_id, trade_type);
			if (map != null) {
				...
				if (return_code.contains("SUCCESS")) {
					...
					if (result_code.contains("SUCCESS")) {
						 ...
						 */
						// 修改会员余额
						MemberBalance ml = memberBalanceDao.get(dRefund.getMemberNo());
						if (ml != null) {
							ml.setMemberDeposit(0D);
							memberBalanceDao.update(ml);
						}
						// 修改押金状态
						DepositStatus ds = depositStatusDao.get(dRefund.getMemberNo());
						if (ds != null) {
							ds.setDepositStatus(0);
							depositStatusDao.update(ds);
						...
						try {
							...
							// 聚通达,浪驰
							/*
							 * sate = sendMsgCommonInterfaceService.sendMsgPost(
							 * phoneNo, censorStatus,
	...
	private Map<String, Object> wxPayRefundRequest(String transaction_id, String out_trade_no, String out_refund_no,
			String total_fee, String refund_fee, String op_user_id, Integer trade_type) throws Exception {
	...
	public ResultInfo<DepositRefund> transAccountByWX(DepositRefund dRefund, DepositOrder dOrder,
		...
		String partner_trade_no = dOrder.getPartTradeFlowNo();
		// 商户appid下，某用户的openid
		...
		try {
			Map<String, Object> map = wxPayTransAccountRequest(mch_appid, mchid, nonce_str, partner_trade_no, openid,
					check_name, amounts, desc, spbill_create_ip, trade_type);
			if (map != null) {
				String return_code = (String) map.get("return_code");
				String result_code = (String) map.get("result_code");
				log.info("WXRefundRes,return_code=" + return_code + ",result_code=" + result_code);
					...
					if (result_code.contains("SUCCESS")) {
						...
						if (dr != null) {
							...
							if (dO != null) {
								...
								accounts.setMemberNo(dO.getMemberNo());
								accounts.setBusinessNo(dO.getDepositOrderNo());
								accounts.setBusinessType(4);
								accounts.setAccountType(2);
								accounts.setAccountMoney(dr.getRefundAmount());
								accounts.setAccountTime(new Date());
	 ...
	 */
	@SuppressWarnings("unchecked")
	public Map<String, Object> wxPayTransAccountRequest(String mch_appid, String mchid, String nonce_str,
			String partner_trade_no, String openid, String check_name, String amounts, String desc,
			String spbill_create_ip, Integer trade_type) throws Exception {
		...
		if (null != current) {
			...
			if (null != order) {
				...
				if (order.getPaymentMethod() == 1) { // 支付方式（1、支付宝、2、微信）
					resultInfo = this.invokingAlipayForGetOrder(order.getPaymentFlowNo(), order.getDepositOrderNo(),
							tradeInfo); // 支付宝交易流水号，本系统唯一支付订单号
				} else {
					resultInfo.setCode(Constant.FAIL);
	...
	@SuppressWarnings("unchecked")
	@Transactional
	private ResultInfo<String> invokingAlipayForGetOrder(String trade_no, String out_trade_no,
		...
		try {
			...
			if ("T".equals(requestInfo)) {
				Iterator<Element> iter = rootElt.elementIterator("response"); // 获取根节点下的子节点alipay
				// 遍历alipay节点
				while (iter.hasNext()) {
					Element recordEle = iter.next();
					Iterator<Element> tradeNode = recordEle.elementIterator("trade"); // 获取子节点response下的子节点trade
					while (tradeNode.hasNext()) {
						result.setCode(Constant.SECCUESS);
						result.setMsg("查询成功");
						Element itemEle = tradeNode.next();
	...
	@Override
	@Transactional
	public ResultInfo<String> refundDepositByAlipay(String depositRefundNo, HttpServletResponse response, String memo)
			throws Exception {
		ResultInfo<String> resultInfo = new ResultInfo<String>();
	...
	@Override
	@Transactional
	public ResultInfo<TransToAccount> transAccountByAlipay(String depositRefundNo, String remark, String payeeId,
		...
		if (null != current && current.getDepositOrderNo() != null && current.getRefundStatus() == 0) {
			...
			if (null != record) {
				...
				AlipayClient alipayClient = new DefaultAlipayClient("https://openapi.alipay.com/gateway.do", appId,
						privateKey, "json", "UTF-8", publicKey, "RSA");
		...
		if (verify_result) {// 验证成功
			...
			if (result_details.indexOf("SUCCESS") > -1 && success_num.equals("1")) {
				...
				try {
					 ...
					 * censorStatus, MsgConstant.MEMBERDEPOSIT);
					 */ // 叮咚云
						// sate =
						// sendMsgCommonInterfaceService.sendMsgPost(mobilePhone,
						// validCode, "","1"); //1代表 验证码类短信
