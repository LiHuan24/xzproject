 ...
 */
@Service
public class RedeemCouponPlanServiceImpl implements RedeemCouponPlanService {

	private static final Log log = LogFactory.getLog(RedeemCouponPlanServiceImpl.class);
	...
	public List<RedeemCouponPlan> getRedeemCouponPlanList(Query q) {
		List<RedeemCouponPlan> list = null;
		try {
			//直接调用Dao方法进行查询
			list = redeemCouponPlanDao.queryAll(q);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
		}
		//如list为null时，则改为返回一个空列表
		list = list == null ? new ArrayList<RedeemCouponPlan>(0) : list;
		return list; 
	}
	...
	public PageFinder<RedeemCouponPlan> getRedeemCouponPlanPagedList(Query q) {
		PageFinder<RedeemCouponPlan> page = new PageFinder<RedeemCouponPlan>();
		...
		try {
			//调用dao查询满足条件的分页数据
			list = redeemCouponPlanDao.pageList(q);
			//调用dao统计满足条件的记录总数
			rowCount = redeemCouponPlanDao.count(q);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
		}
	...
	public RedeemCouponPlan getRedeemCouponPlan(String id) {
		...
		try {
			//调用dao，根据主键查询
			obj = redeemCouponPlanDao.get(id); 
		} catch (Exception e) {
			log.error(e.getMessage(), e);
		}
	...
	public List<RedeemCouponPlan> getRedeemCouponPlanByIds(String[] ids) {
		...
		if (ids == null || ids.length == 0) {
			...
			try {
				//调用dao，根据主键数组查询
				list = redeemCouponPlanDao.getByIds(ids);
			} catch (Exception e) {
				log.error(e.getMessage(), e);
			}
		}
	...
	public ResultInfo<RedeemCouponPlan> delRedeemCouponPlan(String id, Operator operator) {
		ResultInfo<RedeemCouponPlan> resultInfo = new ResultInfo<RedeemCouponPlan>();
		...
		try {
			...
			int count = redeemCouponPlanDao.delete(id);
			if (count > 0) {
	...
	public ResultInfo<RedeemCouponPlan> addRedeemCouponPlan(RedeemCouponPlan redeemCouponPlan, Operator operator) {
		...
		if (redeemCouponPlan == null) { //传入参数无效时直接返回失败结果
			resultInfo.setCode(Constant.FAIL);
			resultInfo.setMsg(Constant.ERR_MSG_INVALID_ARG );
			log.info(Constant.ERR_MSG_INVALID_ARG + " redeemCouponPlan = " + redeemCouponPlan);
		} else {
			try {
				
				//填充默认值
				this.fillDefaultValues(redeemCouponPlan);
				
				//调用Dao执行插入操作
				redeemCouponPlanDao.add(redeemCouponPlan);
				resultInfo.setCode(Constant.SECCUESS);
				resultInfo.setData(redeemCouponPlan);
			} catch (Exception e) {
				log.error(e.getMessage(), e);
				TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
				resultInfo.setCode(Constant.FAIL);
	...
	public ResultInfo<RedeemCouponPlan> updateRedeemCouponPlan(RedeemCouponPlan redeemCouponPlan, Operator operator) {
		...
		if (redeemCouponPlan == null || redeemCouponPlan.getPlanNo() == null) { //传入参数无效时直接返回失败结果
			resultInfo.setCode(Constant.FAIL);
			resultInfo.setMsg(Constant.ERR_MSG_INVALID_ARG );
			log.info(Constant.ERR_MSG_INVALID_ARG + " redeemCouponPlan = " + redeemCouponPlan);
		} else {
			try {
				...
				int count = redeemCouponPlanDao.update(redeemCouponPlan);			
				if (count > 0) {
					resultInfo.setCode(Constant.SECCUESS);
				} else {
					resultInfo.setCode(Constant.FAIL);
