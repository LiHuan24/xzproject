...
import cn.com.shopec.core.themepavilion.dao.LineUpDao;
...
public class LineUpServiceImpl implements LineUpService {
	...
	private static final Log log = LogFactory.getLog(LineUpServiceImpl.class);
	...
	public List<LineUp> getLineUpList(Query q) {
		...
		try {
			//直接调用Dao方法进行查询
			list = lineUpDao.queryAll(q);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
		}
		//如list为null时，则改为返回一个空列表
		list = list == null ? new ArrayList<LineUp>(0) : list;
		return list; 
	}
	...
	public PageFinder<LineUp> getLineUpPagedList(Query q) {
		PageFinder<LineUp> page = new PageFinder<LineUp>();
		...
		try {
			//调用dao查询满足条件的分页数据
			list = lineUpDao.pageList(q);
			//调用dao统计满足条件的记录总数
			rowCount = lineUpDao.count(q);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
		}
	...
	public ResultInfo<LineUp> delLineUp(String id, Operator operator) {
		ResultInfo<LineUp> resultInfo = new ResultInfo<LineUp>();
	...
	public ResultInfo<LineUp> addLineUp(LineUp lineUp, Operator operator) {
		...
		if (lineUp == null) { //传入参数无效时直接返回失败结果
			resultInfo.setCode(Constant.FAIL);
			resultInfo.setMsg(Constant.ERR_MSG_INVALID_ARG );
			log.info(Constant.ERR_MSG_INVALID_ARG + " lineUp = " + lineUp);
		} else {
			try {
				//如果传入参数的主键为null，则调用生成主键的方法获取新的主键
				if (lineUp.getLineUpNo() == null) {
					lineUp.setLineUpNo(this.generatePK());
				}
				//如果传入的操作人不为null，则设置新增记录的操作人类型和操作人id
				if (operator != null) {
					lineUp.setOperatorType(operator.getOperatorType());
					lineUp.setOperatorId(operator.getOperatorId());
				}
				
				//设置创建时间和更新时间为当前时间
				Date now = new Date();
				lineUp.setCreateTime(now);
				lineUp.setUpdateTime(now);
				
				//填充默认值
				this.fillDefaultValues(lineUp);
				
				//调用Dao执行插入操作
				lineUpDao.add(lineUp);
				resultInfo.setCode(Constant.SECCUESS);
				resultInfo.setData(lineUp);
			} catch (Exception e) {
				log.error(e.getMessage(), e);
				TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
				resultInfo.setCode(Constant.FAIL);
	...
	public ResultInfo<LineUp> updateLineUp(LineUp lineUp, Operator operator) {
		...
		if (lineUp == null || lineUp.getLineUpNo() == null) { //传入参数无效时直接返回失败结果
			resultInfo.setCode(Constant.FAIL);
			resultInfo.setMsg(Constant.ERR_MSG_INVALID_ARG );
			log.info(Constant.ERR_MSG_INVALID_ARG + " lineUp = " + lineUp);
		} else {
			try {
				...
				int count = lineUpDao.update(lineUp);			
				if (count > 0) {
					resultInfo.setCode(Constant.SECCUESS);
				} else {
					resultInfo.setCode(Constant.FAIL);
