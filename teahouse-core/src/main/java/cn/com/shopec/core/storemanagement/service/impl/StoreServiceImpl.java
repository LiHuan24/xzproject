package cn.com.shopec.core.storemanagement.service.impl;
...
import cn.com.shopec.core.storemanagement.dao.StoreDao;
...
public class StoreServiceImpl implements StoreService {
	...
	public List<Store> getStoreList(Query q) {
		...
		try {
			// 已删除的不查出
			Store store = (Store) q.getQ();
			if (store != null) {
				store.setIsDeleted(Constant.DEL_STATE_NO);
			}

			// 直接调用Dao方法进行查询
			list = storeDao.queryAll(q);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
		}
		// 如list为null时，则改为返回一个空列表
		list = list == null ? new ArrayList<Store>(0) : list;
		return list;
	}
	...
	public PageFinder<Store> getStorePagedList(Query q) {
		PageFinder<Store> page = new PageFinder<Store>();
		...
		try {
			...
			// 调用dao查询满足条件的分页数据
			list = storeDao.pageList(q);
			// 调用dao统计满足条件的记录总数
			rowCount = storeDao.count(q);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
		}
	...
	public ResultInfo<Store> delStore(String id, Operator operator) {
		...
		try {
			...
			store.setStoreNo(id);
			store.setIsDeleted(Constant.DEL_STATE_YES);
			store.setUpdateTime(new Date());
			if (operator != null) { // 最近操作人
				store.setOperatorType(operator.getOperatorType());
				store.setOperatorId(operator.getOperatorId());
			}

			// 调用Dao执行更新操作，并判断更新语句执行结果
			int count = storeDao.update(store);
			if (count > 0) {
	...
	public ResultInfo<Store> addStore(Store store, Operator operator) {
		...
		if (store == null) { // 传入参数无效时直接返回失败结果
			resultInfo.setCode(Constant.FAIL);
			resultInfo.setMsg(Constant.ERR_MSG_INVALID_ARG);
			log.info(Constant.ERR_MSG_INVALID_ARG + " store = " + store);
		} else {
			try {
				// 如果传入参数的主键为null，则调用生成主键的方法获取新的主键
				if (store.getStoreNo() == null) {
					store.setStoreNo(this.generatePK());
				}
	...
	public ResultInfo<Store> updateStore(Store store, Operator operator) {
		...
		if (store == null || store.getStoreNo() == null) { // 传入参数无效时直接返回失败结果
			resultInfo.setCode(Constant.FAIL);
			resultInfo.setMsg(Constant.ERR_MSG_INVALID_ARG);
			log.info(Constant.ERR_MSG_INVALID_ARG + " store = " + store);
		} else {
	...
	public String generatePK() {
		return "ST" + primarykeyService.getValueByBizType(PrimarykeyConstant.storeNo);
	}
	...
	public Store getStoreById(String entranceCode) {
		return storeDao.getStoreById(entranceCode);
	}
