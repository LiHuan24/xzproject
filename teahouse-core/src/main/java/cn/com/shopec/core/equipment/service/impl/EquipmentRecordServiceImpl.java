...
public class EquipmentRecordServiceImpl implements EquipmentRecordService {
	...
	private static final Log log = LogFactory.getLog(EquipmentRecordServiceImpl.class);
	...
	public List<EquipmentRecord> getEquipmentRecordList(Query q) {
		List<EquipmentRecord> list = null;
		try {
			// 已删除的不查出
			EquipmentRecord equipmentRecord = (EquipmentRecord) q.getQ();
			if (equipmentRecord != null) {
				equipmentRecord.setIsDeleted(Constant.DEL_STATE_NO);
			}

			// 直接调用Dao方法进行查询
			list = equipmentRecordDao.queryAll(q);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
		}
		// 如list为null时，则改为返回一个空列表
		list = list == null ? new ArrayList<EquipmentRecord>(0) : list;
		return list;
	}
	...
	public PageFinder<EquipmentRecord> getEquipmentRecordPagedList(Query q) {
		PageFinder<EquipmentRecord> page = new PageFinder<EquipmentRecord>();
		...
		try {
			...
			// 调用dao查询满足条件的分页数据
			list = equipmentRecordDao.pageList(q);
			// 调用dao统计满足条件的记录总数
			rowCount = equipmentRecordDao.count(q);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
		}
	...
	public List<EquipmentRecord> getEquipmentRecordByIds(String[] ids) {
		...
		if (ids == null || ids.length == 0) {
			...
			try {
				// 调用dao，根据主键数组查询
				list = equipmentRecordDao.getByIds(ids);
			} catch (Exception e) {
				log.error(e.getMessage(), e);
			}
		}
	...
	public ResultInfo<EquipmentRecord> delEquipmentRecord(String id, Operator operator) {
		...
		try {
			...
			equipmentRecord.setRecordId(id);
			equipmentRecord.setIsDeleted(Constant.DEL_STATE_YES);
			equipmentRecord.setUpdateTime(new Date());
			if (operator != null) { // 最近操作人
				equipmentRecord.setOperatorType(operator.getOperatorType());
				equipmentRecord.setOperatorId(operator.getOperatorId());
			}

			// 调用Dao执行更新操作，并判断更新语句执行结果
			int count = equipmentRecordDao.update(equipmentRecord);
			if (count > 0) {
				resultInfo.setCode(Constant.SECCUESS);
			} else {
				resultInfo.setCode(Constant.FAIL);
			}
			resultInfo.setData(equipmentRecord);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
	...
	public ResultInfo<EquipmentRecord> addEquipmentRecord(EquipmentRecord equipmentRecord, Operator operator) {
		...
		if (equipmentRecord == null) { // 传入参数无效时直接返回失败结果
			resultInfo.setCode(Constant.FAIL);
			resultInfo.setMsg(Constant.ERR_MSG_INVALID_ARG);
			log.info(Constant.ERR_MSG_INVALID_ARG + " equipmentRecord = " + equipmentRecord);
		} else {
			try {
				// 如果传入参数的主键为null，则调用生成主键的方法获取新的主键
				if (equipmentRecord.getRecordId() == null) {
					equipmentRecord.setRecordId(this.generatePK());
				}
				...
				// 设置创建时间和更新时间为当前时间
				Date now = new Date();
				equipmentRecord.setCreateTime(now);
				equipmentRecord.setUpdateTime(now);

				// 填充默认值
				this.fillDefaultValues(equipmentRecord);

				// 调用Dao执行插入操作
				equipmentRecordDao.add(equipmentRecord);
				resultInfo.setCode(Constant.SECCUESS);
				resultInfo.setData(equipmentRecord);
			} catch (Exception e) {
				log.error(e.getMessage(), e);
	...
	public ResultInfo<EquipmentRecord> updateEquipmentRecord(EquipmentRecord equipmentRecord, Operator operator) {
		...
		if (equipmentRecord == null || equipmentRecord.getRecordId() == null) { // 传入参数无效时直接返回失败结果
			resultInfo.setCode(Constant.FAIL);
			resultInfo.setMsg(Constant.ERR_MSG_INVALID_ARG);
			log.info(Constant.ERR_MSG_INVALID_ARG + " equipmentRecord = " + equipmentRecord);
		} else {
	...
	@Override
	@Transactional(propagation = Propagation.SUPPORTS)
	public PageFinder<EquipmentRecord> pageListEquipmentRecordByOrderNo(Query q) {
		PageFinder<EquipmentRecord> page = new PageFinder<EquipmentRecord>();
		...
		try {
			...
			// 调用dao查询满足条件的分页数据
			list = equipmentRecordDao.pageListByOrderNo(q);
			// 调用dao统计满足条件的记录总数
			rowCount = equipmentRecordDao.countByOrderNo(q);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
		}
	...
	public EquipmentRecord getErNs(String memberNo, String fitnessEquipmentNo) {
		return equipmentRecordDao.getErNs(memberNo, fitnessEquipmentNo);
	}

	@Override
	public EquipmentRecord getErMs(String orderNo, Integer useStatus) {
		return equipmentRecordDao.getErMs(orderNo, useStatus);
	}
}
