...
public class ECDateUtils {
	...
	// private final static SimpleDateFormat Format_DateTime_WX = new
	// SimpleDateFormat("yyyyMMddHHmmss");
	// private final static SimpleDateFormat Format_DateTime_WX_ALIPAY = new
	// SimpleDateFormat("yyyyMMdd");
	/**
	 * 默认的日期格式化器，格式为yyyy-MM-dd
	 */
	// private final static SimpleDateFormat DEFAULT_DATE_FORMATER = new
	// SimpleDateFormat("yyyy-MM-dd");

	/**
	 * 默认的时间格式化器，格式为yyyy-MM-dd HH:mm:ss
	 */
	// private final static SimpleDateFormat DEFAULT_DATETIME_FORMATER = new
	// SimpleDateFormat("yyyy-MM-dd HH:mm:ss");

	// private final static SimpleDateFormat DEFAULT_DATETIME = new
	...
	public static Date formatDateToDate(Date date) throws ParseException {
		if (date == null)
			...
			// return
			// DEFAULT_DATETIME_FORMATER.parse(DEFAULT_DATETIME_FORMATER.format(date));
	 ...
	 * @throws ParseException
	 */
	public static String farmatDateToHM(Date date) throws ParseException {
	...
	public static Date getCurrentDate() throws ParseException {
		Date date = new Date();
		// date =
		// DEFAULT_DATE_FORMATER.parse(DEFAULT_DATE_FORMATER.format(date));//
		SimpleDateFormat df = new SimpleDateFormat(Format_Date);
	 ...
	 * @return
	 */
	public static String getWeekDayByCurrentDate(Date date) {
		int dateOfWeek = getDayOfWeek(date);
		if (dateOfWeek == Calendar.MONDAY) {
			return "周一";
		} else if (dateOfWeek == Calendar.TUESDAY) {
	...
	public static List<String> getLast12Months(int start, int end) {
		...
		for (int i = 0; i < 12; i++) {
			...
			if (cal.get(Calendar.MONTH) + 1 < 10) {
				if (12 - i < 10) {
					last12Months[i] = cal.get(Calendar.YEAR) + "-0" + (cal.get(Calendar.MONTH) + 1) + "第0" + (12 - i)
							+ "期";
				} else {
					last12Months[i] = cal.get(Calendar.YEAR) + "-0" + (cal.get(Calendar.MONTH) + 1) + "第" + (12 - i)
							+ "期";
				}
			} else {
				if (12 - i < 10) {
					last12Months[i] = cal.get(Calendar.YEAR) + "-" + (cal.get(Calendar.MONTH) + 1) + "第0" + (12 - i)
							+ "期";
				} else {
					last12Months[i] = cal.get(Calendar.YEAR) + "-" + (cal.get(Calendar.MONTH) + 1) + "第" + (12 - i)
							+ "期";
				}
			}
			...
			if (cal.get(Calendar.MONTH) == 0) {
				if (cal.get(Calendar.MONTH) + 1 < 10) {
					last12Months1[i] = (cal.get(Calendar.YEAR) - 1) + "-" + 12 + "-" + start + "~"
							+ cal.get(Calendar.YEAR) + "-0" + (cal.get(Calendar.MONTH) + 1) + "-" + end;
				} else {
					last12Months1[i] = (cal.get(Calendar.YEAR) - 1) + "-" + 12 + "-" + start + "~"
							+ cal.get(Calendar.YEAR) + "-" + (cal.get(Calendar.MONTH) + 1) + "-" + end;
				}

			} else {
				if (cal.get(Calendar.MONTH) < 10) {
					if (cal.get(Calendar.MONTH) + 1 < 10) {
						last12Months1[i] = cal.get(Calendar.YEAR) + "-0" + (cal.get(Calendar.MONTH)) + "-" + start + "~"
								+ cal.get(Calendar.YEAR) + "-0" + (cal.get(Calendar.MONTH) + 1) + "-" + end;
					} else {
						last12Months1[i] = cal.get(Calendar.YEAR) + "-0" + (cal.get(Calendar.MONTH)) + "-" + start + "~"
								+ cal.get(Calendar.YEAR) + "-" + (cal.get(Calendar.MONTH) + 1) + "-" + end;
					...
					if (cal.get(Calendar.MONTH) + 1 < 10) {
						last12Months1[i] = cal.get(Calendar.YEAR) + "-" + (cal.get(Calendar.MONTH)) + "-" + start + "~"
								+ cal.get(Calendar.YEAR) + "-0" + (cal.get(Calendar.MONTH) + 1) + "-" + end;
					} else {
						last12Months1[i] = cal.get(Calendar.YEAR) + "-" + (cal.get(Calendar.MONTH)) + "-" + start + "~"
								+ cal.get(Calendar.YEAR) + "-" + (cal.get(Calendar.MONTH) + 1) + "-" + end;
	...
	public static int getYearSpace(String date1, String date2) throws ParseException {
	...
	/**
	 * 根据当前星期X获取当前星期X对应的日期 Calendar.SUNDAY = 1 Calendar.MONDAY = 2
	 * Calendar.TUESDAY = 3 Calendar.WEDNESDAY = 4 Calendar.THURSDAY = 5
	 * Calendar.FRIDAY = 6 Calendar.SATURDAY = 7
	 */
	public static Calendar getDateByThisDay(int week, int days) {
		Calendar cal = Calendar.getInstance();
		...
		/*
		 * if(isFirstSunday){ day = day - 1; if(day == 0){ day = 7; } }
		 * System.out.println(day);
		 */

		if (day != Calendar.SUNDAY) {
			// week 代表本周还是下周 0：本周 1 下周
		...
		// System.out.println(ECDateUtils.formatStringDate(cal.getTime()));
		return cal;
	}
	...
	/**
	 * 获取指定开始日期和结束日期之间的所有日期
	 * 统计天数的集合
	 * @throws Exception 
	 */
	public static List<String> getDatesForBeginAndEndTime(String beginDate,String endDate) throws Exception{
