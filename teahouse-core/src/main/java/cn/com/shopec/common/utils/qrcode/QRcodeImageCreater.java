...
public class QRcodeImageCreater {
	...
	public static String generateQRCode(String url, int size, String format, String rootName, String resImgPath,
		...
		gs.dispose();
		bufferedImage.flush();
		File outputFileDir = new File("/opt/xz/files/image-server/coach_photo" + File.separator + "qrcode");
		if (!outputFileDir.exists()) {
			outputFileDir.mkdirs();
		}
		
		File newFile = new File(outputFileDir + File.separator + fileName+ ".jpg");
		String resFilePathName = resImgPath + "/" + fileName + "." + format;
		ImageIO.write(bufferedImage, format, newFile);
