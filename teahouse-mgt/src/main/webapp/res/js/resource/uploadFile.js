		...
		function() {
			var upload = {
				...
				init : function(config) {
					// storePath为业务路径，
					if (config.storePath == 'member_icon' || config.storePath == 'equipmentModel_photo'
							|| config.storePath == 'store_photo' || config.storePath == 'couponPlan_photo'
							|| config.storePath == 'equipmentRepair_photo' || config.storePath == 'course_photo'
							|| config.storePath == 'coach_photo'||config.storePath == "advert_photo") {
						$("#" + config.uploadId).empty();
						upload.uploadFile(config);
					} else {
						var form = $("form[name='" + config.formName + "']");
						form
								.find("span[name='" + config.spanName + "']")
								.append(
										"<font color='red' class='formtips onError emaill'>参数storePath不符合规范！</font>");
					}
				},
				uploadFile : function(config) {
					...
					$("#appUpload")
							.load(
									"res/tpl/uploadFile.html",
									function() {
										var manualUploader = new qq.FineUploader(
												{
													element : document
															.getElementById(config.uploadId),
													template : "qq-template-manual-trigger",// "brandUploadFile",
													request : {
														endpoint : upload.appPath
																+ "/upload/uploadFileNew.do",
														method : "post",
														params : {
															storePath : storePath,
													...
													validation : {
														...
														allowedExtensions : config.allowedExtensions,// ["jpeg",
																										// "jpg",
																										// "gif",
																										// "png"],"xls","doc","xlsx","docx","pdf',"txt"],
				...
				deleteFile : function(filePaths, config, fn) {
					...
					$.ajax({
						url : upload.appPath
								+ "/upload/deleteFile.do?filePaths="
								+ filePaths,
						data : {
							resPath : resPath
						},
				...
				picsBindEvent : function(obj, config) {
					obj.find(".qq-upload-up").on("click", function() {
						if (obj.prev().length) {
							 ...
							 * else{ obj.find(".qq-upload-up").hide(); }
							 */
					});
					obj.find(".qq-upload-down").on("click", function() {
						if (obj.next().length) {
							 ...
							 * else{ obj.find(".qq-upload-down").hide(); }
							 */
					});
				...
				getUploadFilePath : function(config) {
					var filePath = [];
					$("#" + config.uploadId + " .qq-upload-success").each(
							function() {
				...
				picsLayout : function(url) {
					...
					li.attr("data-filepath", url).find("img").attr("src",
							imgsrc);//.addClass("qq-file-id-"+x).attr("qq-file-id",x);
