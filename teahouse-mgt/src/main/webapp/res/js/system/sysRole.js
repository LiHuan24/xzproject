...
//	    			async:false,
			...
			pageList:function () {	
				...
				$('#sysRoleList').dataTable( {
				   ...
				   "dom": "<'row'<'#sysRoleTools.col-xs-6'><'col-xs-6'f>r>" +"t" +"<'row'<'col-xs-3'l><'col-xs-3'i><'col-xs-6'p>>",
				   initComplete: function () {
					   $("#sysRoleTools").css("float", "right");
					   $("#sysRoleTools").removeClass("col-xs-6");
					   $(this).find("thead tr th:first-child").prepend('<input type="checkbox">');
						$("#sysRoleTools").append('<button type="button" class="btn btn-default btn-sm btnDefault" data-toggle="modal" data-target="#mySysRoleModal" style="width: 95px; height: 32px; line-height: 25px; -webkit-border-radius: 3px;-moz-border-radius: 3px;border-radius: 3px; background-color: #2b94fd">添加</button>');
