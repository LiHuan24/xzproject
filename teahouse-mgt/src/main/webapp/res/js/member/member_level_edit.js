...
var memberLevelEdit = {
	...
	editMemberLevel:function() {
		...
		if (form.find("input[name='upgradePoint']").val()=="") {
			$("span[name=upgradePointEdit]").append("<font color='red' class='formtips onError emaill'>请输入晋升所需积分！</font>");
			return false;
		}
		...
		var levelName = form.find("input[name='levelName']").val();
		
		$.post("memberLevel/checkMemberLevelUp.do",{memberLevelId:memberLevelId,levelName:levelName},function(res){
			if(res.code==1){
				$("span[name='levelNameEdit']").append("<font color='red'>等级名称重复，请重新输入！</font>");
				return false;
			}else{
				form.ajaxSubmit({
