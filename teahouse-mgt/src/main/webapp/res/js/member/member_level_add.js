...
var memberLevelAdd = {
	...
	addMemberLevel:function() {
		...
		if (form.find("input[name='upgradePoint']").val()=="") {
			$("span[name=upgradePointAdd]").append("<font color='red' class='formtips onError emaill'>请输入晋升所需积分！</font>");
			return false;
		}
		...
		$.post("memberLevel/checkMemberLevel.do",{levelName:levelName},function(res){
			if(res.code==1){
				$("span[name='levelNameAdd']").append("<font color='red'>等级名称重复，请重新输入！</font>");
				return false;
			}else{
