...
var couponAdd = {
		...
		init : function() {
			...
			//新增图片回填
			$("#addCouponPicBtn").click(function(){
...
/*			if ($("#memberNo").val()=="") {
				$("span[name='memberNoAdd']").append("<font color='red'>请选择会员！</font>");
				return false;
			}*/
		}
	});
	...
	$('#couponLists').dataTable( {
	   ...
	   "dom": "<'row'<'#couponToolss.col-xs-6'><'col-xs-6'f>r>" +"t" +"<'row'<'col-xs-3'l><'col-xs-3'i><'col-xs-6'p>>",
	   initComplete: function () {
		   	$(this).find("thead tr th:first-child").prepend('');
		   	/*$("#couponTools").append('<button type="button"  class="btn btn-default btn-sm coupan-batch-sel">绑定优惠券</button>');
			$("#couponTools").append('<button type="button"  class="btn btn-default btn-sm coupan-batch-close">关闭</button>');
			$(".coupan-batch-sel").on("click",function(){
				$('#couponLists thead input[type="checkbox"]').on("click",function(e){
	   				if(this.checked){
				         $('#couponLists tbody input[type="checkbox"]:not(:checked)').prop("checked",true);
				    } else {
				         $('#couponLists tbody input[type="checkbox"]:checked').prop("checked",false);
				    }
				});
			});*/
			$("#couponToolss").append('<button type="button"  class="btn btn-default btn-sm coupan-batch-sel">确认</button>');
			$("#couponToolss").append('<button type="button"  class="btn btn-default btn-sm coupan-batch-close">关闭</button>');
			$(".coupan-batch-sel").on("click",function(){
				$("#couponModal").modal("hide");
				$('#couponLists tbody input[type="checkbox"]:checked').prop("checked",false);
			});
			$(".coupan-batch-close").on("click",function(){
				$("#couponModal").modal("hide");
			});
		},
			 ...
			 columnDefs:[
			    {
					...
					"render" : function(data, type, full, meta) {
						var text = '<input type="checkbox"  name="idNo" value="' + full.planNo + '" nameValue="'+full.title+'" ';
			 ...
			 columnDefs:[
				 {
					...
					"orderable":false,
					"render" : function(data, type, full, meta) {
						var text = '<input type="checkbox"  name="idNo" value="' + full.memberNo + '" nameValue="'+full.memberName+'" ';
						var data = couponAdd.getSelectedData(dataType);
						for(var i = 0; i < data.length;i ++){
		...
		$.ajax({
			...
			success:function(plan){
				if(plan != null){ 
					...
					}else if(plan.couponMethod == 3){
						form.find("input[name=couponMethodName]").val("社区馆一天体验券");
						 form.find(".couponMethod-1").hide(); 
						 form.find(".couponMethod-2").hide(); 
						 form.find(".couponMethod-4").hide(); 
						 form.find(".couponMethod-3").show(); 
//						 form.find("input[name=discount]").val("");
						 form.find("input[name=freeFitnessTime]").val(plan.freeFitnessTime)
					}else if(plan.couponMethod == 4){
						form.find("input[name=couponMethodName]").val("主题课程一节体验券");
						 form.find(".couponMethod-1").hide(); 
						 form.find(".couponMethod-2").hide(); 
						 form.find(".couponMethod-3").hide(); 
						 form.find(".couponMethod-4").show(); 
//						 form.find("input[name=discount]").val("");
						 form.find("input[name=freeCourseNumber]").val(plan.freeCourseNumber)
					}
					form.find("input[name=discount]").val(plan.discount);
					form.find("input[name=discountAmount]").val(plan.discountAmount);
					...
					}else if(plan.availableTime1 != null){
						form.find(".availableTime-1").hide(); 
						form.find(".availableTime-2").show(); 
						form.find("input[name=availableDays]").val("");
						form.find("input[name=availableTime1]").val(couponAdd.getDate(plan.availableTime1));
						form.find("input[name=availableTime2]").val(couponAdd.getDate(plan.availableTime2));
					}
	...
	$("input[name=idNo]").click(function(a,b,c,d){
		...
		$("#couponLists tbody").find("input:checked").each(function(){
		...
		$("#couponLists tbody").find("input[type=checkbox]").not("input:checked").each(function(){
    ...
    day = oDate.getDate(),  
    oTime = year +'-'+ couponAdd.getzf(month) +'-'+  couponAdd.getzf(day);
    return oTime;  
},getzf: function(num){  
    if(parseInt(num) < 10){  
        num = '0'+num;  
