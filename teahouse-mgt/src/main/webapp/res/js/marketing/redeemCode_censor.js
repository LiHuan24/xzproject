...
var redeemCodeCensor = {
		...
		init : function() {
			...
			if(redeemCode == null){
				...
				redeemCode = form.find("input[name='redeemCode']").val();
			}
			$('.couPonPlanLists_censor').dataTable( {
				...
				"ajax": {
					"type": "POST",
					"url": redeemCodeCensor.appPath+'/redeemCode/getPageForRedeemmCode.do?redeemCode='+redeemCode,
					"data": function ( d ) {
						return $.extend( {}, d, {
				...
				"columns": [
				   			   ...
				   			   { "title":"兑换数量","data": "redeemQuantity" }
			...
			var form = $("form[name=redeemCodeCensorForm]");
			form.ajaxSubmit({
				url : redeemCodeCensor.appPath + "/redeemCode/updateRedeemCode.do",
				type : "post",
