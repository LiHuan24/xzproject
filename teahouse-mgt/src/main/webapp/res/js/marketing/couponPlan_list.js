	...
	var couponPlan={
				...
				$(".couponPlan-operate-available").each(function(id,obj){
					$(this).on("click",function(){
						...
						bootbox.confirm("<img src='res/img/wen.png' style='width: 29px;height: 29px;margin-top: -4px'>&nbsp;&nbsp;" + "确定要"+hint+"优惠券吗？", function(result) {
							if(result){
								 $.ajax({
	    				    			url:couponPlan.appPath+"/couponPlan/updateCouponPlan.do",
	    				    			type:"post",
	    				    			data:{
	    				    				planNo:id,
				...
				$(".couponPlan-operate-delete").each(function(id,obj){
					$(this).on("click",function(){
						...
						bootbox.confirm("<img src='res/img/wen.png' style='width: 29px;height: 29px;margin-top: -4px'>&nbsp;&nbsp;" + "确定要删除优惠券吗？", function(result) {
							if(result){
								 $.ajax({
	    				    			url:couponPlan.appPath+"/couponPlan/deleteCouponPlan.do",
	    				    			type:"post",
	    				    			data:{
	    				    				planNo:id,
	    				    				isDeleted:1//删除标识
			...
			pageList:function () {	
				...
				$('#couponPlanList').dataTable( {
					...
					"ajax": {
						...
						"data": function ( d ) {
							...
							return $.extend( {}, d, {
	        					...
	        					"couponMethod":form.find("select[name=couponMethod]").val(),
				   ...
				   "dom": "<'row'<'#couponPlanTools.col-xs-6'><'col-xs-6'f>r>" +"t" +"<'row'<'col-xs-3'l><'col-xs-3'i><'col-xs-6'p>>",
				   initComplete: function () {
					   	$("#couponPlanTools").html("");
					   	$("#couponPlanTools").css("float", "right");
					   	$("#couponPlanTools").removeClass("col-xs-6");
					   	$("#couponPlanTools").append('<button type="button" class="btn btn-default btn-sm couponPlanTools-operate-add btnDefault" style="width: 95px; height: 32px; line-height: 25px; -webkit-border-radius: 3px;-moz-border-radius: 3px;border-radius: 3px; background-color: #2b94fd" >新增</button>');
