	...
	var depositRefund={
			...
			init: function () {	
				...
				$("#closeDepositRefundMemo").click(function(){
					...
					$("form[name=refundDepositByOfflineForm]").resetForm();
			    	$("form[name=refundDepositByOfflineForm]").find("textarea[name=refundMemo]").val("");
				});
				
			},
			memoDepositRefund:function() {
				var form = $("form[name=refundDepositByOfflineForm]");
				form.ajaxSubmit({
					url : depositRefund.appPath + "/depositRefund/refundDepositByOffline.do",
					type : "post",
	        						...
	        						if(res.data){
	        							...
	        							if(status==0||status==2){
	        								if(type==1){//支付宝
	        	    	        				...
	        	    	        				var formMemo = $("form[name=refundDepositByOfflineForm]");
...
//	       				});
			...
			pageList:function () {
				...
				$('#depositRefundList').dataTable( {
				   ...
				   "dom": "<'row'<'#depositRefundTools.col-xs-6'><'col-xs-6'f>r>" +"t" +"<'row'<'col-xs-3'l><'col-xs-3'i><'col-xs-6'p>>",
				   initComplete: function () {
					   $("#depositRefundTools").html("");
					   $("#depositRefundTools").css("float", "right");
					   $("#depositRefundTools").removeClass("col-xs-6");
					   $("#depositRefundTools").append('<button type="button" class="btn btn-default btn-sm depositRefundTools-operate-export btnDefault" style="width: 95px; height: 32px; line-height: 25px; -webkit-border-radius: 3px;-moz-border-radius: 3px;border-radius: 3px; background-color: #2b94fd" >导出</button>');
		       			/**
		       			 * 导出
		       			 */
		       			$(".depositRefundTools-operate-export").on("click",function(){
		       				bootbox.confirm("<img src='res/img/wen.png' style='width: 29px;height: 29px;margin-top: -4px'>&nbsp;&nbsp;" + "确定要导出吗？", function(result) {
								if(result){
									var form = $("form[name=depositRefundSearchForm]");
									form.attr("action",depositRefund.appPath+ "/depositRefund/toDepositRefundExport.do").submit();
//									window.location.href = depositRefund.appPath+ "/depositRefund/toDepositRefundExport.do?filepath=depositRefund&newFileName=depositRefund.xls";
								}						
		       				});
		       			}); 	     			
